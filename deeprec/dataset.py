...
class RecDataset:
    ...
    def set_params(self, user_id:str=None, item_id:str=None, target_id:str=None):
    ...
    def __call__(
        ...
        if isinstance(user_features,pd.DataFrame):
            X = X.merge(
                user_features,
                left_on=self.user_id,
                right_on=self.user_id,
                how='left'
            )
        ...
        if isinstance(item_features, pd.DataFrame):
            X = X.merge(
                item_features,
                left_on=self.item_id,
                right_on=self.item_id,
                how='left'
            )
